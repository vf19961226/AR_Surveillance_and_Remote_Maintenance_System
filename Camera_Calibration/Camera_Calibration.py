...
cap = cv.VideoCapture(0, cv.CAP_DSHOW) #cv2.CAP_DSHOW據說為Windows特有，不知道Linux需不需要，主要是拿來消除影像黑邊（0代表第一台相機）
cap.set(cv.CAP_PROP_FRAME_WIDTH, width) #設定相機影像寬度
cap.set(cv.CAP_PROP_FRAME_HEIGHT, height) #設定相機影像高度
cap.set(cv.CAP_PROP_FPS, fps) #設定相機取像頻率
...
while(1):
    ...
    if ret == True :
        ...
        if cv.waitKey(1) & 0xFF == ord('q'): 
            cv.imwrite("./figure/img" + str(i) + ".jpg", frame)
            i = i + 1
            if  i == photo_num:
                break
...
# 去畸变
img2 = cv.imread("./figure/img0.jpg")
h,  w = img2.shape[:2]
...
'''將結果寫入Numpy檔'''
np.savez('./Camera_Parameter.npz', mtx=mtx, dist=dist, rvecs=rvecs, tvecs=tvecs) 